...
export const apiBudgetFindAllPagination = async (search: Search): Promise<PaginationResponse<BudgetResponse>> => {
...
export const apiBudgetFindById = async (id: number): Promise<BudgetResponse> => {
    const url = BE_BUDGET + "/id/" + id;
    const headers = await createHeadersWithSession();
    const response = await makeGetRequest(url, headers);
    return await handleResponse(response);
};

export const apiBudgetCreate = async (request: BudgetRequest): Promise<BudgetResponse> => {
    const headers = await createHeadersWithSession();
    const response = await makePostRequest(BE_BUDGET, headers, request);
    return await handleResponse(response);
};

export const apiBudgetUpdate = async (id: number, request: BudgetRequest): Promise<BudgetResponse> => {
    const headers = await createHeadersWithSession();
    const response = await makePutRequest(id, BE_BUDGET, headers, request);
    return await handleResponse(response);
};

export const apiBudgetDelete = async (id: number): Promise<BudgetResponse> => {
    const headers = await createHeadersWithSession();
    const response = await makeDeleteRequest(id, BE_BUDGET, headers);
    return await handleResponse(response);
};

export const apiBudgetRestore = async (id: number): Promise<BudgetResponse> => {
